...
System.register(['./vender/d3'], function (_export, _context) {
	"use strict";

	var d3;
	function directive_gauge($timeout) {
		return {
			restrict: 'E',
			scope: {
				data: '=',
			...
			link: function link(scope, element, attrs) {
				...
				scope.$watch('data', function () {
					if (gauge_created) g.redraw(scope.data);else {
						recreate_gauge();
					}
				}, true);
	...
	_export('directive_gauge', directive_gauge);
	...
	function Gauge(placeHolder, gauge_size, config, data) {
		...
		function valueToDegrees(value) {
			return value / pointer.range * 240 - 30;
		}
		...
		function scale(value) {
			if (value >= this.max) return 1;else if (value <= this.min) return 0;else return (value - this.min) / this.range;
		};
		this.render = function () {
			...
			var body = d3.select('#' + self.placeHolder).append("svg").attr("id", "svg-" + self.placeHolder).attr('width', gauge_size).attr('height', gauge_size);
			...
			clip = defs.append("clipPath").attr('id', 'rectclip_' + self.placeHolder).append("rect").attr('x', 0).attr('height', gauge_size).attr('width', gauge_size);
			circle_s = body.append("circle").attr("cx", gauge_size / 2).attr("cy", gauge_size / 2).attr("r", gauge_size / 2 - 3);
			circle_c = body.append('circle').attr("cx", gauge_size / 2).attr("cy", gauge_size / 2).attr("r", gauge_size / 2 - 3).attr('clip-path', 'url(' + baseUrl + '#rectclip_' + self.placeHolder + ')').attr("style", "stroke:rgb(0,0,0);stroke-width:0;z-index:10000;");
			title_f = body.append("text").attr('x', gauge_size / 2).attr('style', 'font-weight:bold;text-anchor:middle;').style('font-size', gauge_size / 17);
			title_m = body.append('text').attr('x', gauge_size / 2).attr('y', gauge_size / 4).attr('style', 'font-weight:bold;text-anchor:middle;').style('font-size', gauge_size / 15).text(pointer.name);
			title_v = body.append('text').attr('x', gauge_size / 2).attr('y', gauge_size / 8 * 3).attr('style', 'font-weight:bold;text-anchor:middle').style('font-size', gauge_size / 10);
			...
			_.each(_.range(0, pointer.range + 1, pointer.range / 4), function (tck) {
				...
				tickers.append('line').attr('x1', pt1.x).attr('y1', pt1.y).attr('x2', pt2.x).attr('y2', pt2.y).attr('style', 'stroke:#333;stroke-width:2px');
				if (tck < pointer.max) {
					_.each(_.range(tck, tck + pointer.range / 4, pointer.range / 20), function (tck2) {
						...
						tickers.append('line').attr('x1', pt1.x).attr('y1', pt1.y).attr('x2', pt2.x).attr('y2', pt2.y).attr('style', 'stroke:#333;stroke-width:1px');
					});
				}
			});
			svg_pointer = body.append('g');
			svg_pointer.selectAll('path').data([buildPointerPath(pointer.range / 2)]).enter().append('path').attr('d', d3.svg.line().x(function (d) {
				return d.x;
			}).y(function (d) {
				return d.y;
			}).interpolate('basis')).attr('style', 'stroke:#c63110;fill-opacity:0.6');
		...
		this.redraw = function (data) {
			...
			circle_s.data([data]).attr("style", function (d) {
			...
			title_v.data([data]).text(function (d) {
				return Math.round(d.pointer);
			});
			svg_pointer.data([data]).attr('transform', function (d) {
